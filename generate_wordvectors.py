...
expr_nan_set=df.loc[pd.isnull(df['Experiences'])];
print("Experiences nan number: ",len(expr_nan_set));
indices=expr_nan_set.index;
df=df.drop(df.index[[indices]]);         

shape_nan_set=df.loc[pd.isnull(df['Shapes'])];
...
print(df.head(2))
#state examples
state_nan_set=df.loc[pd.isnull(df['State'])];
...
print("After: ",df.ix[state_nan_set.index,:]);
state_nan_set=df.loc[pd.isnull(df['State'])];
#remove those indices that are not resolved yet.
print("still nan states number: ",len(state_nan_set));
...
print(len(country_nan_set))

states= country_nan_set['State']
print len(np.unique(states.values))
print(country_nan_set)
